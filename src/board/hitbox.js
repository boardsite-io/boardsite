export function getHitbox(
    ...
    let xy1 = [
        Math.floor(positions[0] * reduceLoadFactor),
        Math.floor(positions[1] * reduceLoadFactor),
    ]
    let xy2
    let hitbox = []
    for (
        let i = 2 * pointSkipFactor;
        i < positions.length;
        i += 2 * pointSkipFactor
    ) {
        xy2 = [
            Math.floor(positions[i] * reduceLoadFactor),
            Math.floor(positions[i + 1] * reduceLoadFactor),
        ]
        if (
            Math.pow(xy2[0] - xy1[0], 2) + Math.pow(xy2[1] - xy1[1], 2) <
            quadMinPixDist
        ) {
            // move to next iter if points too close
            continue
        }
    ...
    xy2 = [
        Math.floor(positions[positions.length - 2] * reduceLoadFactor),
        Math.floor(positions[positions.length - 1] * reduceLoadFactor),
    ]
    if (
        Math.pow(xy2[0] - xy1[0], 2) + Math.pow(xy2[1] - xy1[1], 2) <
        quadMinPixDist
